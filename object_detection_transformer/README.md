...
- `scripts/api_server.(sh|ps1)` starts the FastAPI bridge for dataset uploads, training triggers, and dashboard metric sync.
...
- For TPU training on Kaggle, copy `kaggle_fusionnet_notebook.py` into a notebook and set `FUSIONNET_BACKEND_URL` / `FUSIONNET_API_TOKEN` environment variables to sync checkpoints back to the Codespaces backend.
...
- API notes: use `scripts/api_server.sh` (Linux/macOS/Codespaces) or `scripts/api_server.ps1` (Windows) to launch the FastAPI bridge. Set `FUSIONNET_API_TOKEN` before launch and reuse that token in Kaggle (`FUSIONNET_API_TOKEN` env var) for uploads and dashboard metric pushes.
